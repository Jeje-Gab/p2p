...
CREATE TABLE IF NOT EXISTS offers (
    ...
    skin_offered_id BIGINT NOT NULL REFERENCES skins(id) ON DELETE CASCADE,
    skin_requested_id BIGINT NOT NULL REFERENCES skins(id) ON DELETE CASCADE,
...
CREATE INDEX idx_offers_owner ON offers(owner_id) WHERE deleted_at IS NULL;
...
COMMENT ON TABLE offers IS 'P2P trade offers created by users';
COMMENT ON COLUMN offers.status IS 'Offer lifecycle: open (available) -> accepted (trade executed) or cancelled (user cancelled)';
COMMENT ON TABLE trades IS 'Historical record of completed trades for audit trail';
