...
CREATE INDEX idx_users_steam_id ON users(steam_id) WHERE steam_id IS NOT NULL AND deleted_at IS NULL;

-- Create login_attempts table for brute force protection
CREATE TABLE IF NOT EXISTS login_attempts (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT REFERENCES users(id) ON DELETE SET NULL, -- Nullable if user doesn't exist
...
COMMENT ON TABLE users IS 'User accounts with password hashing (bcrypt/argon2), 2FA support, and role-based access';
COMMENT ON COLUMN users.password_hash IS 'Bcrypt or Argon2id hash - NEVER store passwords in plain text!';
COMMENT ON COLUMN users.twofa_secret IS 'TOTP secret for Google Authenticator - provides second factor authentication';
COMMENT ON TABLE login_attempts IS 'Tracks login attempts for brute force detection and mitigation';
